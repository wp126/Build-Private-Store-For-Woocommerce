        ...
        function custom_pre_get_posts_query( $q ) {
          ...
          if($bpsfw_comman['bpsfw_enable_private_whole_website'] != 'yes' && $bpsfw_comman['bpsfw_disble_price_addtocartbutton'] != "yes"){
            ...
            if (!empty($productsa) || empty($productsa) && !empty($appended_terms) || empty($appended_terms)) {
              $appended_tags = get_option('wg_tags_select2');
        ...
        function bpsfw_hide_price_addcart_not_logged_in( $price, $product ) {
          ...
          if($bpsfw_comman['bpsfw_enable_private_whole_website'] != "yes" && $bpsfw_comman['bpsfw_disble_price_addtocartbutton'] == "yes"  || !empty(get_option('wg_combo'))){
            if ( ! is_user_logged_in() ) { 
                ...
                remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_add_to_cart', 30 );
              }else if(!empty(get_option('wg_cats_select2'))){
                      ...
                      $terms = get_the_terms ( get_the_id(), 'product_cat');
                        ...
                        foreach ($terms as $key => $value) {
                            ...
                            if(!empty(get_option('wg_cats_select2'))){
                                 // print_r($value->term_id);
                                if (in_array($value->term_id, get_option('wg_cats_select2'))) {
              ...
              }else if(!empty(get_option('bpsfw_tags_select2'))){
                  ...
                  if(!empty($terms)){
                    ...
                    foreach ($terms as $key => $value) {
                        ...
                        if(!empty(get_option('bpsfw_tags_select2'))){
                             // print_r($value->term_id);
                            if (in_array($value->term_id, get_option('bpsfw_tags_select2'))) {
